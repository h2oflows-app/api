...
LOCAL_PGURL="postgres://h2oflows:h2oflows@localhost:5432"
DUMP="/tmp/h2oflows-prod-$(date +%Y%m%d-%H%M%S).dump"
...
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=accept-new "$PROD_HOST" \
  "cd $PROD_DIR && docker compose -f docker-compose.prod.yml exec -T postgres \
   pg_dump -U h2oflows -Fc h2oflows" \
  > "$DUMP"
...
psql "${LOCAL_PGURL}/postgres?sslmode=disable" -c "DROP DATABASE IF EXISTS h2oflows;"
psql "${LOCAL_PGURL}/postgres?sslmode=disable" -c "CREATE DATABASE h2oflows OWNER h2oflows;"
