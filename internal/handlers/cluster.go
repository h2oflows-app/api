	...
	communityQ := `
		SELECT
			...
			(
				...
				-- plans.visibility='public' required so a paddled run inside a
				-- private plan can't inflate a public reach's discovery rank.
				+ COALESCE((SELECT COUNT(*) FROM plan_runs pr JOIN plans pl ON pl.id = pr.plan_id AND pl.deleted_at IS NULL AND pl.visibility = 'public' WHERE pr.user_reach_id = ur.id AND pr.paddled AND pr.deleted_at IS NULL), 0) * 2
			...
			COALESCE((SELECT COUNT(*) FROM plan_runs pr JOIN plans pl ON pl.id = pr.plan_id AND pl.deleted_at IS NULL AND pl.visibility = 'public' WHERE pr.user_reach_id = ur.id AND pr.paddled AND pr.deleted_at IS NULL), 0)::int AS report_count
