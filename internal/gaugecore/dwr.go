...
type dwrReading struct {
	...
	QualityType  string  `json:"qualityType"`  // "Approved", "Provisional", "Observed", etc.
	FlagA        *string `json:"flagA"`        // legacy flag field — fallback if qualityType absent
}
...
func (r *dwrReading) toReading(externalID string) (*Reading, error) {
	...
	// qualityType is the canonical field; flagA is a fallback for older responses.
	qualCode := r.QualityType
	if qualCode == "" && r.FlagA != nil {
		qualCode = *r.FlagA
	}
	provisional := !strings.EqualFold(qualCode, "Approved") && qualCode != "A" && qualCode != "a"
