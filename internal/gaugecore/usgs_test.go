...
func TestUSGSSource_FetchReading(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "provisional reading",
			body:       `{"value":{"timeSeries":[{"variable":{"unit":{"unitCode":"ft3/s"},"noDataValue":-999999.0},"values":[{"value":[{"value":"850.0","qualifiers":["P"],"dateTime":"2024-03-25T14:00:00.000+00:00"}]}]}]}}`,
			statusCode: 200,
			wantValue:  850.0,
			wantUnit:   "cfs",
			wantProvis: true,
		...
		{
			name:       "approved reading",
			body:       `{"value":{"timeSeries":[{"variable":{"unit":{"unitCode":"ft3/s"},"noDataValue":-999999.0},"values":[{"value":[{"value":"340.0","qualifiers":["A"],"dateTime":"2024-03-25T14:00:00.000+00:00"}]}]}]}}`,
			statusCode: 200,
			wantValue:  340.0,
			wantUnit:   "cfs",
			wantProvis: false,
